...
namespace encoding_rs {
	...
	public:
		...
		inline const Encoding* encoding() const {
			return static_cast<const Encoding*>(decoder_encoding(this));
		}
		 ...
		 */
		inline std::optional<size_t> max_utf8_buffer_length_without_replacement(size_t byte_length) const {
			size_t val = decoder_max_utf8_buffer_length_without_replacement(this, byte_length);
			if (val == SIZE_MAX) {
				return std::nullopt;
			}
			return val;
		...
		inline std::tuple<uint32_t, size_t, size_t, bool> decode_to_utf8(
		     ztd::span<const uint8_t> src, ztd::span<uint8_t> dst, bool last) {
			size_t src_read    = src.size();
			size_t dst_written = dst.size();
			bool had_replacements;
			uint32_t result = decoder_decode_to_utf8(this, null_to_bogus<const uint8_t>(src.data()), &src_read,
			     null_to_bogus<uint8_t>(dst.data()), &dst_written, last, &had_replacements);
		...
		inline std::tuple<uint32_t, size_t, size_t> decode_to_utf8_without_replacement(
			...
			size_t dst_written = dst.size();
			uint32_t result
			     = decoder_decode_to_utf8_without_replacement(this, null_to_bogus<const uint8_t>(src.data()),
		...
		inline std::tuple<uint32_t, size_t, size_t, bool> decode_to_utf16(
		     ztd::span<const uint8_t> src, ztd::span<char16_t> dst, bool last) {
			size_t src_read    = src.size();
			size_t dst_written = dst.size();
			bool had_replacements;
			uint32_t result = decoder_decode_to_utf16(this, null_to_bogus<const uint8_t>(src.data()), &src_read,
			     null_to_bogus<char16_t>(dst.data()), &dst_written, last, &had_replacements);
			return { result, src_read, dst_written, had_replacements };
		}
		...
		inline std::tuple<uint32_t, size_t, size_t> decode_to_utf16_without_replacement(
			...
			size_t dst_written = dst.size();
			uint32_t result
			     = decoder_decode_to_utf16_without_replacement(this, null_to_bogus<const uint8_t>(src.data()),
			          &src_read, null_to_bogus<char16_t>(dst.data()), &dst_written, last);
			return { result, src_read, dst_written };
		}
		 ...
		 */
		inline std::optional<size_t> latin1_byte_compatible_up_to(ztd::span<const uint8_t> buffer) const {
			size_t val = decoder_latin1_byte_compatible_up_to(
			     this, null_to_bogus<const uint8_t>(buffer.data()), static_cast<size_t>(buffer.size()));
			if (val == SIZE_MAX) {
				return std::nullopt;
			}
			return val;
	...
	public:
		 ...
		 */
		inline const Encoding* encoding() const {
			return static_cast<const Encoding*>(encoder_encoding(this));
		}
		 ...
		 */
		inline std::optional<size_t> max_buffer_length_from_utf8_if_no_unmappables(size_t byte_length) const {
			size_t val = encoder_max_buffer_length_from_utf8_if_no_unmappables(this, byte_length);
			if (val == SIZE_MAX) {
				return std::nullopt;
			}
			return val;
		 ...
		 */
		inline std::optional<size_t> max_buffer_length_from_utf8_without_replacement(size_t byte_length) const {
			size_t val = encoder_max_buffer_length_from_utf8_without_replacement(this, byte_length);
			if (val == SIZE_MAX) {
				return std::nullopt;
			}
			return val;
		...
		inline std::tuple<uint32_t, size_t, size_t, bool> encode_from_utf8(
		     std::string_view src, ztd::span<uint8_t> dst, bool last) {
			size_t src_read    = src.size();
			size_t dst_written = dst.size();
			bool had_replacements;
			uint32_t result = encoder_encode_from_utf8(this,
			     null_to_bogus<const uint8_t>(reinterpret_cast<const uint8_t*>(src.data())), &src_read,
			     null_to_bogus<uint8_t>(dst.data()), &dst_written, last, &had_replacements);
			return { result, src_read, dst_written, had_replacements };
		}
		...
		inline std::tuple<uint32_t, size_t, size_t> encode_from_utf8_without_replacement(
			...
			size_t src_read    = src.size();
			size_t dst_written = dst.size();
			uint32_t result    = encoder_encode_from_utf8_without_replacement(this,
			        null_to_bogus<const uint8_t>(reinterpret_cast<const uint8_t*>(src.data())), &src_read,
		 ...
		 * the input or `SIZE_MAX` if `size_t` would overflow.
		 */
		inline std::optional<size_t> max_buffer_length_from_utf16_if_no_unmappables(size_t u16_length) const {
			size_t val = encoder_max_buffer_length_from_utf16_if_no_unmappables(this, u16_length);
			if (val == SIZE_MAX) {
				return std::nullopt;
			}
			return val;
		 ...
		 * additional input code units or `SIZE_MAX` if `size_t` would overflow.
		 */
		inline std::optional<size_t> max_buffer_length_from_utf16_without_replacement(size_t u16_length) const {
			size_t val = encoder_max_buffer_length_from_utf16_without_replacement(this, u16_length);
			if (val == SIZE_MAX) {
				return std::nullopt;
			}
			return val;
		...
		inline std::tuple<uint32_t, size_t, size_t, bool> encode_from_utf16(
		     std::u16string_view src, ztd::span<uint8_t> dst, bool last) {
			size_t src_read    = src.size();
			size_t dst_written = dst.size();
			bool had_replacements;
			uint32_t result = encoder_encode_from_utf16(this, null_to_bogus<const char16_t>(src.data()), &src_read,
			     null_to_bogus<uint8_t>(dst.data()), &dst_written, last, &had_replacements);
			return { result, src_read, dst_written, had_replacements };
		}
		...
		inline std::tuple<uint32_t, size_t, size_t> encode_from_utf16_without_replacement(
			...
			size_t dst_written = dst.size();
			uint32_t result
			     = encoder_encode_from_utf16_without_replacement(this, null_to_bogus<const char16_t>(src.data()),
			          &src_read, null_to_bogus<uint8_t>(dst.data()), &dst_written, last);
			return { result, src_read, dst_written };
		}
	...
	public:
		 ...
		 */
		static inline const Encoding* for_label(ztd::c_string_view label) {
			return encoding_for_label(
			     null_to_bogus<const uint8_t>(reinterpret_cast<const uint8_t*>(label.c_str())), label.size());
		}
		 ...
		 */
		static inline const Encoding* for_label_no_replacement(ztd::c_string_view label) {
		 ...
		 */
		static inline std::optional<std::tuple<const Encoding*, size_t>> for_bom(ztd::span<const uint8_t> buffer) {
			size_t len               = buffer.size();
			const Encoding* encoding = encoding_for_bom(null_to_bogus(buffer.data()), &len);
			if (encoding) {
				return std::make_tuple(static_cast<const Encoding*>(encoding), len);
			}
			return std::nullopt;
		}
		...
		inline const Encoding* output_encoding() const {
			return static_cast<const Encoding*>(encoding_output_encoding(this));
		}
		 ...
		 */
		inline std::tuple<std::string, const Encoding*, bool> decode(ztd::span<const uint8_t> bytes) const {
			...
			return { str, static_cast<const Encoding*>(encoding), had_errors };
		}
		 ...
		 */
		inline std::tuple<std::string, bool> decode_with_bom_removal(ztd::span<const uint8_t> bytes) const {
			if (this == UTF_8_ENCODING && bytes.size() >= 3
			     && (ztd::as_bytes(bytes.first<3>()) == ztd::as_bytes(ztd::make_span("\xEF\xBB\xBF")))) {
				bytes = bytes.subspan(3, bytes.size() - 3);
			}
			else if (this == UTF_16LE_ENCODING && bytes.size() >= 2
			     && (ztd::as_bytes(bytes.first<2>()) == ztd::as_bytes(ztd::make_span("\xFF\xFE")))) {
				bytes = bytes.subspan(2, bytes.size() - 2);
			}
			else if (this == UTF_16BE_ENCODING && bytes.size() >= 2
			     && (ztd::as_bytes(bytes.first<2>()) == ztd::as_bytes(ztd::make_span("\xFE\xFF")))) {
				bytes = bytes.subspan(2, bytes.size() - 2);
			}
		 ...
		 */
		inline std::tuple<std::string, bool> decode_without_bom_handling(ztd::span<const uint8_t> bytes) const {
			...
			const auto [result, read, written, had_errors] = decoder->decode_to_utf8(
			     bytes, ztd::make_span(reinterpret_cast<uint8_t*>(&string[0]), string.size()), true);
		 ...
		 * `new_decoder_without_bom_handling()` when decoding segmented input.
		 */
		inline std::optional<std::string> decode_without_bom_handling_and_without_replacement(
		     ztd::span<const uint8_t> bytes) const {
			auto decoder = new_decoder_without_bom_handling();
			auto needed  = decoder->max_utf8_buffer_length_without_replacement(bytes.size());
			if (!needed) {
				throw std::overflow_error("Overflow in buffer size computation.");
			}
			std::string string(needed.value(), '\0');
			const auto [result, read, written] = decoder->decode_to_utf8_without_replacement(
			     bytes, ztd::make_span(reinterpret_cast<uint8_t*>(&string[0]), string.size()), true);
		 ...
		 * when decoding segmented input.
		 */
		inline std::tuple<std::u16string, const Encoding*, bool> decode16(ztd::span<const uint8_t> bytes) const {
			auto opt = Encoding::for_bom(bytes);
			const Encoding* encoding;
			if (opt) {
				size_t bom_length;
		 ...
		 * `new_decoder_with_bom_removal()` when decoding segmented input.
		 */
		inline std::tuple<std::u16string, bool> decode16_with_bom_removal(ztd::span<const uint8_t> bytes) const {
			if (this == UTF_8_ENCODING && bytes.size() >= 3
			     && (ztd::as_bytes(bytes.first<3>()) == ztd::as_bytes(ztd::make_span("\xEF\xBB\xBF")))) {
				bytes = bytes.subspan(3, bytes.size() - 3);
			}
		 ...
		 * `new_decoder_without_bom_handling()` when decoding segmented input.
		 */
		inline std::tuple<std::u16string, bool> decode16_without_bom_handling(ztd::span<const uint8_t> bytes) const {
			auto decoder = new_decoder_without_bom_handling();
			...
			const auto [result, read, written, had_errors]
			     = decoder->decode_to_utf16(bytes, ztd::make_span(&string[0], string.size()), true);
			assert(read == static_cast<size_t>(bytes.size()));
			assert(written <= static_cast<size_t>(string.size()));
			assert(result == INPUT_EMPTY);
			string.resize(written);
		 ...
		 * `new_decoder_without_bom_handling()` when decoding segmented input.
		 */
		inline std::optional<std::u16string> decode16_without_bom_handling_and_without_replacement(
		     ztd::span<const uint8_t> bytes) const {
			auto decoder = new_decoder_without_bom_handling();
			auto needed  = decoder->max_utf16_buffer_length(bytes.size());
			if (!needed) {
			...
			std::u16string string(needed.value(), '\0');
			const auto [result, read, written] = decoder->decode_to_utf16_without_replacement(
			     bytes, ztd::make_span(&string[0], string.size()), true);
			assert(result != OUTPUT_FULL);
			if (result == INPUT_EMPTY) {
				assert(read == static_cast<size_t>(bytes.size()));
				assert(written <= static_cast<size_t>(string.size()));
		 ...
		 */
		inline std::tuple<std::vector<uint8_t>, const Encoding*, bool> encode(std::string_view string) const {
			...
			for (;;) {
				const auto [result, read, written, had_errors] = encoder->encode_from_utf8(
				     string.substr(total_read), ztd::make_span(vec).subspan(total_written), true);
				total_read += read;
		...
		inline std::tuple<std::vector<uint8_t>, const Encoding*, bool> encode(std::u16string_view string) const {
			...
			size_t total_written  = 0;
			for (;;) {
				const auto [result, read, written, had_errors] = encoder->encode_from_utf16(
				     string.substr(total_read), ztd::make_span(vec).subspan(total_written), true);
				total_read += read;
				total_written += written;
				total_had_errors |= had_errors;
		...
		inline std::unique_ptr<Decoder> new_decoder_with_bom_removal() const {
			return std::unique_ptr<Decoder>(encoding_new_decoder_with_bom_removal(this));
		}
		...
		inline std::unique_ptr<Decoder> new_decoder_without_bom_handling() const {
			return std::unique_ptr<Decoder>(encoding_new_decoder_without_bom_handling(this));
		}
		 ...
		 */
		static inline size_t utf8_valid_up_to(ztd::span<const uint8_t> buffer) {
			return encoding_utf8_valid_up_to(null_to_bogus<const uint8_t>(buffer.data()), buffer.size());
		}
		 ...
		 */
		static inline size_t ascii_valid_up_to(ztd::span<const uint8_t> buffer) {
			return encoding_ascii_valid_up_to(null_to_bogus<const uint8_t>(buffer.data()), buffer.size());
		}
		 ...
		 * ISO-2022-JP.
		 */
		static inline size_t iso_2022_jp_ascii_valid_up_to(ztd::span<const uint8_t> buffer) {
